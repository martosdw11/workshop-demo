...
import { deleteOwnResponse, updateOwnResponse } from '@/server/services/response.service';

/**
 * Edit/hapus respons milik sendiri — semua tipe (jawaban, komentar, issue).
 *
 ...
 * pemanggil, `403 ENROLLMENT_COMPLETED` setelah finish (§4.5).
...
export const PATCH = withHandler<Params>(async (req, ctx) => {
  ...
  return ok({ response: await updateOwnResponse(responseId, user, input) });
});
...
export const DELETE = withHandler<Params>(async (_req, ctx) => {
  ...
  const { responseId } = parseParams(ctx.params, paramsSchema);
  await deleteOwnResponse(responseId, user);
